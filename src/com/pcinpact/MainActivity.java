...
//import com.pcinpact.views.OnRefreshListener;
//import com.pcinpact.views.PullToRefreshListView;
...
public class MainActivity extends SherlockActivity implements IConnectable, OnItemClickListener  
{
		...
		switch (pItem.getItemId())
		{
			case 0:
				if(!listView.isRefreshing())
					listView.setRefreshing();// loadArticlesListFromServer(); Refreshes by its own				
				return true;
		}
    ...
    void loadArticles()
    {	
    	adapter.refreshData(PCInpact.getInstance(this).getArticlesWrapper().getArticles());
    }
    ...
    void deleteOldArticles()
    {
    	   ...
    	   for(String file : SavedFiles)
    	   {
    		   ...
    		   for(INpactArticleDescription article : PCInpact.getInstance(this).getArticlesWrapper().getArticles())
	       		{	
	...
	{	
		...
		PCInpact.getInstance(this).getArticlesWrapper().LastUpdate =  Calendar.getInstance().getTime().toLocaleString();
		PCInpact.getInstance(this).getArticlesWrapper().setArticles(articles);
		ArticleManager.saveArticlesWrapper(this, PCInpact.getInstance(this).getArticlesWrapper());
		
		loadArticles();
		
		headerTextView.setText("Dernière mise à jour : "+PCInpact.getInstance(this).getArticlesWrapper().LastUpdate);
    	...
    	for(int i=0; i<articles.size();i++)
    	{
    		...
    		connector.sendRequest(PCInpact.PC_INPACT_URL+article.getUrl(), "GET", null, 0, null);
    	}
    	...
    	for(int i=0; i<articles.size();i++)
    	{
    		...
    		{	
	            ...
	            connector.sendRequest(PCInpact.PC_INPACT_URL+"/comment/", "POST", data, null);
    		}
    	}
	...
	{	 
		...
		else if(state== DL_LIST)
		{
	        ...
	        {	
	        	Log.e("WTF", "could not load list " +  e.getMessage());
	        	stopRefreshing();
	...
	{	
		...
		{   
			...
			Log.e("didFailWithErrorOnUiThread", "state: " +state + " error :"+ error+"");// error is nil!
	}
	...
	{	
		...
		//int index = PCInpact.getInstance(this).getArticlesWrapper().getArticles().indexOf(article);
